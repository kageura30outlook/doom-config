...
#+end_src
* Keybind notes
describe what each keybindings does.



Here's a summary of keybindings in your Doom Emacs configuration along with their descriptions:

- *SPC o P*: Org Pomodoro - Start a Pomodoro timer in Org mode.
- *SPC w*: Window Hydra - Opens a hydra for window management.
  - *n*: Next window.
  - *p*: Previous window.
  - *v*: Split vertically.
  ...
  - *d*: Delete window.
  - *m*: Maximize window.
- *SPC if*: Insert file name - Insert the selected file name at the current point.
- *SPC ir*: Insert relative file name - Insert the relative filename of the selected file.
- *SPC d p*: Toggle peep-dired-mode - Enter peep mode in Dired.
- *SPC c g*: Run GPTel - Launch the GPTel command interface.
- *SPC m g*: GPTel menu - Open the GPTel menu.
- *SPC r g*: GPTel rewrite - Rewrite command in GPTel.
- *SPC s g*: GPT chat - Send a chat message in GPTel.
- *SPC o t*: Toggle vterm popup - Open or close an inline vterm popup.
- *SPC o T*: Open inline vterm - Open a new inline vterm in the current buffer.
- *SPC X*: Quick access to Org agenda - Open the Org agenda.
- *SPC a*: Toggle agend commands - Access different agenda views (day, week, month, etc.).
- *SPC n f*: Find node - Look up notes in Org-roam.
- *SPC n i*: Insert node - Insert a note in Org-roam.
- *SPC n d n*: Capture today's daily note - Create a new daily note.
